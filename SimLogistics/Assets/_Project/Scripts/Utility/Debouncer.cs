...
public class DebounceDispatcher : DebounceDispatcher<bool>
{
    ...
    /// <summary>
    ///     The call of the Action that will be invoked after waiting for the time interval if there are no more calls in this
    ///     interval
    /// </summary>
...
public class DebounceDispatcher<T>
{
    ...
    /// </summary>
    /// <param name="interval">
    ///     The interval in milliseconds that must elapse after the last call so that a Function to be
    ///     invoked
    /// </param>
    ...
    /// <summary>
    ///     The call of the Function that will be invoked after waiting for the time interval if there are no more calls in
    ///     this interval
    /// </summary>
    /// <param name="functToInvoke">Function that will be invoked after waiting for the time interval</param>
    /// <returns>
    ///     Task with a result that will complete when Function will be invoked. All tasks will return the same value if
    ///     the invoking occurs once
    /// </returns>
    public Task<T> DebounceAsync(Func<Task<T>> functToInvoke)
    {
        lock (locker)
        {
            ...
            waitingTask = Task.Run(() =>
            {
                do
                {
                    var delay = (int) (interval - (DateTime.UtcNow - lastInvokeTime).TotalMilliseconds);
